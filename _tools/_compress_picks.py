# -*- coding: utf-8 -*-
"""压缩32课选定动图到 assets/yuque_web/ 目录（宽520、帧≤32、调色板160）"""
import os
...
for lid, (folder, uuid) in sorted(PICKS.items()):
    ...
    for fr in ImageSequence.Iterator(img):
        if idx % step == 0:
            ...
            frames.append(f.convert('P', palette=Image.ADAPTIVE, colors=160))
    ...
    total_bytes += kb
    print(f'L{lid:02d}: {os.path.getsize(p)//1024//1024}MB -> {kb}KB ({len(frames)}帧)')
